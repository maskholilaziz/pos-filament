    ...
    <div class="grid grid-cols-12 gap-6">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 max-h-[60vh] overflow-y-auto pr-2">
                    @forelse ($this->products as $product)
                        <div wire:click="addToCart({{ $product->id }})"
                            class="relative cursor-pointer rounded-lg border bg-white p-3 shadow-sm transition hover:border-primary-500 hover:ring-2 hover:ring-primary-500">
                            <p class="font-semibold text-gray-800">{{ $product->name }}</p>
                            <p class="text-sm font-bold text-primary-600">
                                {{ \Illuminate\Support\Number::currency($product->selling_price, 'IDR', 'id') }}
                            </p>
                            <span
                                class="absolute -top-2 -right-2 inline-flex items-center rounded-full bg-gray-100 px-2 py-1 text-xs font-medium text-gray-600 ring-1 ring-inset ring-gray-500/10">
                                Stok: {{ $product->stock }}
                            </span>
                        </div>
                    @empty
        ...
        <div class="col-span-12 md:col-span-5">
            <x-filament::card class="flex h-full flex-col">
                ...
                <div class="flex-grow space-y-3 overflow-y-auto py-4 pr-2">
                        ...
                        <div class="flex items-center justify-between gap-4 rounded-lg border p-3">
                            ...
                            <div class="flex items-center gap-2">
                                <button wire:click="updateQuantity({{ $productId }}, 'decrement')"
                                    class="rounded-md border p-1 hover:bg-gray-100">
